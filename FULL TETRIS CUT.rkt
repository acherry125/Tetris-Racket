...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |FULL TETRIS CUT|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
(require 2htdp/image)
...
(define ROW-WORLD (make-world L-START 
                              (list (make-block 9 19 'red)
                                    (make-block 8 19 'red)
                                    (make-block 7 19 'red)
                                    (make-block 6 19 'red)
                                    (make-block 5 19 'red)
...
(check-random (next-world COLLIDED-WORLD) 
              (make-world  ((λ (n)(get-tetra (random 7))) 4)
                           (list
                            (make-block 3 1 'red) 
                            (make-block 4 1 'red)
                            (make-block 4 0 'red)
...
(check-expect (next-world (make-world S-START (list (make-block 9 19 'red)
                                                    (make-block 8 19 'red)
                                                    (make-block 7 19 'red)
                                                    (make-block 6 19 'red)
                                                    (make-block 5 19 'red)
                                                    ...
                                                    (make-block 2 18 'blue)
                                                    (make-block 4 18 'blue)
                                                    (make-block 4 17 'yellow))
                                      100))
              (make-world S-START (list (make-block 0 19 'blue)
                                        (make-block 2 19 'blue)
                                        (make-block 4 19 'blue)
                                        (make-block 4 18 'yellow))
                          100))

(define (next-world w)
  (local ((define (update-score a-bset)
            (cond
              [(collide-y? w) (+ 4 (world-score w))]
              [else (world-score w)])))
    (cond
      [(lose? (world-pile w)) (drop-tetra w 0 (world-score w))]
      [(collide-y? w) 
       (make-world (get-tetra (random 7)) 
                   (append (tetra-blocks (world-tetra w)) (world-pile w))                                 
                   (+ (length (tetra-blocks (world-tetra w))) 
                      (world-score w)))]
      [(> (length (get-all-rows-of-length BOARD-WIDTH 
                                          (get-all-rows (world-pile w)))) 0)
       (make-world (world-tetra w) 
                   (check-remove-move (world-pile w)) 
                   (world-score w))]
      [else (drop-tetra 
             w DEFAULT-SPEED (update-score w))])))
...
;;; check-remove-move : BSet -> BSet
;;; Removes all full rows from the pile and moves the rows above down.
(check-expect (check-remove-move empty) empty)
(check-expect (check-remove-move (list
                                  (make-block 0 1 'blue)
                                  (make-block 4 5 'blue)
                                  (make-block 0 8 'blue)
                                  (make-block 1 8 'blue)
              ...
              (list
               ...
               (make-block 4 6 'blue)
               (make-block 0 2 'blue)))
(define (check-remove-move pile)
  (if (empty? pile)
      pile
      (move-down-above (get-all-rows-of-length 10 (get-all-rows pile))
                       (remove-full-rows pile))))
...
(define (get-all-rows bset)
  (cond [(empty? bset) empty]
        [else (cons (get-block-row (first bset) bset)
                    (get-all-rows (filter (λ (b) 
                                            (not (= (block-y b) 
                                                    (block-y (first bset))))) 
                                          bset)))]))
;;; get-block-row : Block BSet -> BSet
;;; Gets all of the blocks in the same row as the given block
(check-expect (get-block-row (make-block 4 4 'blue) empty) empty)
(check-expect (get-block-row (make-block 0 4 'blue) 
                             (list (make-block 0 4 'blue)
                                   (make-block 2 3 'blue)
                                   (make-block 3 3 'blue)
                                   (make-block 2 4 'blue)))
              (list (make-block 0 4 'blue) (make-block 2 4 'blue)))

(define (get-block-row block bset)
  (filter (λ (b) (= (block-y b) (block-y block)))
          bset))

;;; move-down-above : [List-of BSet] [List-of BSet] -> BSet
;;; moves all rows above the list of removed rows down by one
(check-expect (move-down-above (list (list (make-block 0 3 'blue)))
                               (list (list (make-block 0 1 'blue) 
                                           (make-block 1 1 'blue))
                                     (list (make-block 0 2 'blue))
                                     (list (make-block 0 4 'blue))))
              (list (make-block 0 4 'blue) (make-block 0 3 'blue) 
                    (make-block 0 2 'blue) (make-block 1 2 'blue)))

(define (move-down-above lob-of-removed lob)
  (foldr append empty (move-down-above-y 
                       (map (λ (row) 
                              (block-y (first row))) lob-of-removed)
                       lob)))

;;; move-down-above-y : [List-of Number] [List-of BSet] -> [List-of BSet]
;;; Moves down every row above the give y.
(check-expect (move-down-above-y 
               (list 4 5)
               (list (list (make-block 0 0 'red))
                     (list (make-block 0 1 'red))
                     (list (make-block 0 3 'red))
                     (list (make-block 0 6 'red))))
              (list
               (list (make-block 0 6 'red))
               (list (make-block 0 4 'red))
               (list (make-block 0 2 'red))
               (list (make-block 0 1 'red))))
(define (move-down-above-y ys rows)
  (append (get-all-rows-below (first (sort ys >)) rows) 
          (move-down-all-ys (first (sort ys >))
                            rows)))

;;; move-down-all-ys : Number [List-of BSet] -> BSet
;;; Moves each row of y in the list down by 1.
(check-expect (move-down-all-ys
               3
               (list (list (make-block 4 2 'green))
                     (list (make-block 4 3 'green))))
              (list (list (make-block 4 4 'green))
                    (list (make-block 4 3 'green))))

(define (move-down-all-ys ys rows)
  (cond
    [(= ys -1) empty]
    [else (append
           (add-to-all-rows-of-y 1 ys rows)
           (move-down-all-ys (sub1 ys) rows))]))

;;; add-to-all-rows-of-y : Number Number [List-of BSet] -> [List-of BSet]
;;; Adds number to the y of every row of the given y.
(check-expect (add-to-all-rows-of-y 1 1 empty) empty)
(check-expect (add-to-all-rows-of-y 1 2 (list 
                                         (list (make-block 0 1 'blue))
                                         (list (make-block 0 2 'red)
                                               (make-block 1 2 'red))
                                         (list (make-block 4 4 'green))))
              (list (list (make-block 0 3 'red)
                          (make-block 1 3 'red))))

(define (add-to-all-rows-of-y add y rows)
  (local (;;; get-row-y : Number [List-of BSet] -> [List-of BSet]
          ;;; Gets the row with the given y.
          (define (get-row-y y lob)
            (filter
             (λ (bset) (= y (block-y (first bset))))
             lob)))
    (if (not (empty? rows))
        (map (lambda (b) (add-to-bset-y b add))
             (get-row-y y rows))
        rows)))

;;; get-all-rows-below : Number [List-of BSet] -> [List-of BSet]
;;; gives each of the rows bellow y
(check-expect (get-all-rows-below 3 (list
                                     (list (make-block 0 1 'green))
                                     (list (make-block 0 2 'green))
                                     (list (make-block 0 4 'green))))
              (list (list (make-block 0 4 'green))))
(define (get-all-rows-below y rows)
  (filter (λ (row) (< y (block-y (first row))))
          rows))
...
;;; get-all-rows-of-length : Number [List-of BSet] -> [List-of BSet]
;;; Selects the BSets of the given length.
(check-expect (get-all-rows-of-length 4 (list (list 3 4 5 2) (list 2 1) 
                                              (list 1000 339)))
              (list (list 3 4 5 2)))
(check-expect (get-all-rows-of-length 2 (list (list 1) (list 2 3 4)))
              empty)

(define (get-all-rows-of-length num lob)
  (filter (λ (row) (= (length row) num))
          lob))
...
(check-expect (delete-row (list (list (make-block 2 2 'green))
                          ...
                          (list 
                           (list (make-block 3 4 'green))
                           (list (make-block 2 3 'blue))
                           (list (make-block 2 2 'green)))) 
              (list (list (make-block 2 3 'blue))))

(define (delete-row subset set)
  (filter (λ (bset) (not (member? bset subset))) set))
...
(check-expect (key-handler (make-world (make-tetra 
                                        (make-posn 5 5)
...
(define (rotate-bset dir posn bset)
  (local (;; block-rotate-cw : Posn Block -> Block
    ...
    (if (or (not (symbol? dir)) (empty? bset)) bset
        (map (λ (block) (block-rotate dir posn block)) bset))))
...
(define (collide-y? world)
  (local (;; block-adjacent-y? : Block Block -> Boolean
          ...
          (define (bset-adjacent-y? bset1 bset2)
            ...
            (ormap (λ (b-block) (ormap (λ (a-block) 
...
(define (side-wall-collide? dir bset)
  (ormap (λ (block) (cond [(symbol=? 'left dir) (<= (block-x block) 0)]
...
(define (side-bset-collide? dir tetra-bset pile)
  (local ((define (plus-or-minus dir) (cond [(symbol=? dir 'left) -]
    ...
    (ormap (λ (b-block) 
             (ormap (λ (a-block) (block-adjacent-x dir b-block a-block)) pile)) 
           tetra-bset)))
...
(define (lose? bset)
  (ormap (λ (a-block) (= (block-y a-block) 0)) bset))
...
(define (drop-tetra w speed score)
  (make-world ((λ (speed1) (update-tetra (world-tetra w) speed1)) speed)
              (world-pile w)
              score))
