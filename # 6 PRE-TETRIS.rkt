...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |# 6 PRE-TETRIS|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
(require 2htdp/image)
...
;;; Template for a function that takes a block
...
;;; This is a template for a function that takes a tetra
#; (define (tetra-temp a-tetra)
     (... (tetra-center a-tetra) ... (tetra-blocks a-tetra)))
...
;;; This is a template for a function that takes a BSet
#; (define (bset-temp a-bset)
     (cond
       [(empty? a-bset) ...]
       [else (first a-bset) ... (bset-temp (rest a-bset)) ...]))
...
;;; This is a template for a function that takes a world
#; (define (world-temp world)
     (... (tetra-temp (world-tetra world)) ... 
          (bset-temp (world-pile world)) 
          ...(world-score world)))
...
(check-expect 
 (place-bset (list (make-block 4 0 'limegreen) 
                   ...
                   (make-block 5 1 'limegreen)) 
             BOARD)
 (place-image/grid (square 10 "outline" 'black)
                   4
                   0
                   (place-image/grid 
                    (square 10 "solid" 'limegreen) 
                    4
                    0
                    (place-image/grid 
...
(define (place-bset bset image)
  (cond [(empty? bset) image]
        [else (place-image/grid (square 10 "outline" 'black)
                                (block-x (first bset))
                                (block-y (first bset))  
...
(define (place-score score image)
  (place-image/grid (text (number->string score) 20 'black)
                    7
                    1
                    image))
...
(define (display-end world)
  (place-image/grid 
   ...
   (place-image/grid (text "SCORE" 25 'black) 4.5 7
                     (place-image/grid 
                      (text (number->string (world-score world)) 45 'black)
                      4.5 
                      10
